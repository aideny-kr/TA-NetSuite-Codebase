...
//			}
	...
	if (type=='create'){
		...
		if(executionContext == 'webstore' && (orderStatus == 'G' || orderStatus == 'B')) {
			var param = {};
			param['custscript_soid'] = +soid;
			nlapiScheduleScript('customscript_ta_sitebuilder_webuser_sche', 'customdeploy1', param)
		}
...
function createWebUserForSiteBuilder_Sched(type) {
	...
	var soid = context.getSetting('SCRIPT', 'custscript_soid');
	
	var soRec = nlapiLoadRecord('salesorder', soid);
	var customerRec = nlapiLoadRecord('customer', soRec.getFieldValue('entity'));
	var contact = customerRec.getLineItemValue('contactroles', 'contact', 1);
	var user_flds_val = nlapiLookupField('contact', contact, ['email', 'firstname', 'lastname']);
	var lineNumber = soRec.getLineItemCount('item');
	...
	for (var i = 1; i <= lineNumber; i += 1) {
		items_to_commit.push(soRec.getLineItemValue('item', 'item', i));
	}
	
	if(items_to_commit.length > 0) {
		var webUserRec = nlapiCreateRecord('customrecord_contractuser');
		webUserRec.setFieldValue('custrecord_contractuser_customer', customer);
		webUserRec.setFieldValue('custrecord_contractuser_contract', contract);  
		webUserRec.setFieldValue('custrecord_contractuser_contact', contact);
		webUserRec.setFieldValue('custrecord_contractuser_email', user_flds_val['email']);
		webUserRec.setFieldValue('custrecord_contractuser_firstname', user_flds_val['firstname']);
		webUserRec.setFieldValue('custrecord_contractuser_lastname', user_flds_val['lastname']);
		webUserRec.setFieldValues('custrecord_contractuser_items', items_to_commit);
		webUserRec.setFieldValue('custrecord_contractuser_activeflag', 'T');
		nlapiSubmitRecord(webUserRec, true);
	}
