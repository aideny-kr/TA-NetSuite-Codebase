...
function ccerp_PrintMailingLabels(request,response){
	...
	if(request.getMethod() == 'GET'){
		...
		form.addField('handdelivery','checkbox','Hand Delivery');
		response.writePage(form);
	}
	else{
		var items = request.getParameterValues('items');
		var handdelivery = request.getParameter('handdelivery');
		if(handdelivery != 'T')
			handdelivery = 'F';
		nlapiLogExecution('debug', 'ccerp_PrintMailingLabels','items '+JSON.stringify(items)+' items.length '+items.length+' handdelivery '+handdelivery);
		
		var noresults = true;
		var labellines = [];
		var contractids = [];
		var uniquecontractids = [];
		var mlfils = [];
		...
		if(isNotEmpty(mlresults)){
			...
			for(var i=0;i<mlresults.length;i++){
				...
				if(completecontractid == true){
					...
					if(uniquecontractids.indexOf(contractid)==-1){
						contractids.push({
							'contractid' : contractid,
							'address' : address,
							'zip' : zip
						});
						uniquecontractids.push(contractid);
					}
