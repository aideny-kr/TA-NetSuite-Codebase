...
function taCaseAfterSubmit(type){
	if(type == 'edit' || type == 'create') {
		...
		if(isNotEmpty(user_email) && nlapiGetFieldValue('company') == '5') {
			...
			if(company_id) nlapiSubmitField('supportcase', nlapiGetRecordId(), ['company', 'email'], [company_id, user_email]);
			else nlapiSubmitField('supportcase', nlapiGetRecordId(), ['company', 'email'], ['5', user_email]);
		}
	}
}